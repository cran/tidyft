...
If you try to do data operations on any data.table(s), never use it again for futher analysis, because it is not the data you knew before. And you might never figure out what have happened and what have been changed in that process. If you really want to use it again, try make a copy first by using `copy()`, which might take extra time and space (that's why tidyft avoid doing this all the time).<br><br>
